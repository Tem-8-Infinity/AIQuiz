  ...
  return (
    <div className="bg-border bg-gradient-to-br from-teal-400 to-teal-10 p-2 min-h-screen">
      <div className="flex items-center">
        {user?.role === "admin" && (
          <button
            className={`btn bg-gradient-to-br from-violet-400 to-teal-200 text-black font-bold border-none mb-3 m-4`}
            onClick={() => {
              setFilter(!filter);
            }}
          >
            {filter ? "Display All Quizzes" : "Display only my Quizzes"}
          </button>
        )}
        {quizzes.length === 0 ? (
      ...
      {(filter
        ? filterQuiz.filter((quiz) => quiz.createdBy === user.username)
        : filterQuiz
      ).map((quiz, index) => (
        <div
          key={index}
        ...
        >
          <div className="flex flex-wrap m-5 ">
            <button
              className="btn bg-gradient-to-br from-violet-400 to-teal-200 text-black font-bold border-none "
              onClick={() => {
            ...
            <button
              className="btn bg-gradient-to-br from-violet-400 to-teal-200 text-black font-bold border-none ml-1"
              onClick={async () => {
